...
public class Vector {
    ...
    public double angle() {
        if(this.angle!=360.0)
            return this.angle;
        if(this.y==0)
            angle= (this.x<0)?180.0:0.0;
        else if(this.x==0)
            angle= 90.0 + (this.y<0?180.0:0.0);
        else {
	        double angle = Math.toDegrees(Math.asin(this.y/this.magnitude()));
	        if(this.y<0) {
	            if(this.x<0) {
	                angle += 180.0;
	            } else {
	                angle = -1.0 * angle + 360.0;
	            }
	        } else {
	            if(this.x<0) {
	                angle = -1.0 * angle + 180.0;
	            }
	        }
        }
        this.angle = angle;
